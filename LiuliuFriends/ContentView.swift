...
private struct TargetArea: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .temperature:
                VStack(spacing: metrics.targetContentSpacing) {
                    TemperatureTargetView(temperature: round.targetTemperature ?? .warm, accentColor: round.mode.accentColor)
                        .frame(width: metrics.targetIconSize * 1.12, height: metrics.targetIconSize * 0.92)
                    TargetCaption(title: "找\(round.targetTemperature?.speechTitle ?? "冷暖朋友")", mode: round.mode, isCompact: metrics.isCompact)
                }
                .padding(.vertical, metrics.targetVerticalInset)
            case .brightness:
                VStack(spacing: metrics.targetContentSpacing) {
                    BrightnessTargetView(brightness: round.targetBrightness ?? .bright, accentColor: round.mode.accentColor)
                        .frame(width: metrics.targetIconSize * 1.12, height: metrics.targetIconSize * 0.92)
                    TargetCaption(title: "找\(round.targetBrightness?.speechTitle ?? "明暗朋友")", mode: round.mode, isCompact: metrics.isCompact)
                }
                .padding(.vertical, metrics.targetVerticalInset)
            case .weight:
                VStack(spacing: metrics.targetContentSpacing) {
                    WeightTargetView(weight: round.targetWeight ?? .heavy, accentColor: round.mode.accentColor)
                        .frame(width: metrics.targetIconSize * 1.12, height: metrics.targetIconSize * 0.92)
                    TargetCaption(title: "找\(round.targetWeight?.speechTitle ?? "轻重朋友")", mode: round.mode, isCompact: metrics.isCompact)
                }
                .padding(.vertical, metrics.targetVerticalInset)
            case .material:
                VStack(spacing: metrics.targetContentSpacing) {
...
private struct GameObjectView: View {
    ...
    var body: some View {
        ...
        case .vehicle, .fruit, .shape, .colorShape, .body, .sense, .clothing, .vegetable, .food, .tableware, .hygiene, .home, .stationery, .instrument, .toy, .nature, .place, .profession, .object, .size, .purpose, .safety, .habit, .scene, .samePlace, .weather, .season, .routine, .action, .texture, .temperature, .brightness, .weight, .material, .taste, .pairing, .animalHome, .animalBaby, .animalFood, .itemHome, .origin, .pattern, .difference:
            FriendShape(kind: candidate.kind, color: candidate.color, isShadow: false)
        case .emotion:
...
private struct HabitTargetView: View {
    let habit: FriendHabit
    let accentColor: Color

    var body: some View {
        ConceptTargetCard(
            title: habit.promptTitle,
            subtitle: "好习惯",
...
private struct TemperatureTargetView: View {
    let temperature: FriendTemperature
    let accentColor: Color

    var body: some View {
        ConceptTargetCard(
            title: temperature.promptTitle,
            subtitle: "冷和暖",
            systemName: temperature.iconName,
            accentColor: accentColor,
            motif: .weather
        )
    }
...
private struct BrightnessTargetView: View {
    let brightness: FriendBrightness
    let accentColor: Color

    var body: some View {
        ConceptTargetCard(
            title: brightness.promptTitle,
            subtitle: "亮和暗",
            systemName: brightness.iconName,
            accentColor: accentColor,
            motif: .spark
        )
    }
...
private struct WeightTargetView: View {
    let weight: FriendWeight
    let accentColor: Color

    var body: some View {
        ConceptTargetCard(
            title: weight.promptTitle,
            subtitle: "轻和重",
            systemName: weight.iconName,
            accentColor: accentColor,
            motif: .quantity
        )
    }
...
private struct ItemHomeTargetView: View {
    let itemHome: FriendItemHome
    let accentColor: Color

    var body: some View {
        ConceptTargetCard(
            title: itemHome.promptTitle,
            subtitle: "放在哪里",
            systemName: itemHome.iconName,
            accentColor: accentColor,
            motif: .bubble
        )
    }
